...
namespace Private {
    ...
    class TTcpServer : public ITcpContext {
    public:
        TTcpServer(asio::io_context& ioContext)
        ...
        virtual asio::awaitable<void> AsyncAccept(std::shared_ptr<asio::ip::tcp::acceptor> acceptor) {
            try
            {
                for (;;)
                {
                    std::shared_ptr<T> newSocket = std::make_shared<T>(*this);
    ...
    protected:
        virtual bool AcquireSocket(FTcpSocket* tcpSocket) override
        {
            BOOST_ASSERT(Strand.running_in_this_thread());
            auto& endpoint = tcpSocket->GetEndpointRef();
            auto insertResult = SocketMap.insert(std::make_pair(std::make_pair(endpoint.address().to_v4().to_uint(), endpoint.port()), std::static_pointer_cast<T>(tcpSocket->shared_from_this())));
    ...
    protected:
        ...
        std::map<FSocketId, std::weak_ptr<T>> SocketMap;

    };

    class FRpcServer : public TTcpServer<FRpcSocket> {
    public:
        FRpcServer(asio::io_context& ioContext)
            : TTcpServer(ioContext)
        {}
